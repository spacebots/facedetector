...
void FaceDetector::StartFaceDetection() {
  cv::VideoCapture capture(m_camId);

  if (!capture.isOpened()) {
    std::cerr << "Failed to capture from device " << m_camId << std::endl;
    return;
  }
  ...
  while (true) {
    cv::Mat frame;
    capture >> frame;
    DetectAndDraw(frame, m_dScale);
    char key = (char)cv::waitKey(10);
    switch (key) {
      case -1: break;
      case 'B': m_criteria |= CV_HAAR_FIND_BIGGEST_OBJECT; std::cout << "FIND BIGGEST: ON\n"; break;
      case 'b': m_criteria  = CV_HAAR_FIND_BIGGEST_OBJECT; break;
      case 'P': m_criteria |= CV_HAAR_DO_CANNY_PRUNING; break;
      case 'p': m_criteria  = CV_HAAR_DO_CANNY_PRUNING; break;
      case 'R': m_criteria |= CV_HAAR_DO_ROUGH_SEARCH; break;
      case 'r': m_criteria  = CV_HAAR_DO_ROUGH_SEARCH; break;
      case 'S': m_criteria |= CV_HAAR_SCALE_IMAGE; break;
      case 's': m_criteria  = CV_HAAR_SCALE_IMAGE; break;
      case '0': m_criteria = 0; break;
      case 'f': m_drawFrame = !m_drawFrame; break;
      case 'n': m_drawName = !m_drawName; break;
      case 'c': m_writeFrames = !m_writeFrames; break;
...
void FaceDetector::DetectAndDraw(cv::Mat &frame, double m_dScale) {
  cv::Mat gray(frame.cols, frame.rows, 8, 1);
  cv::cvtColor(frame, gray, CV_BGR2GRAY);

  cv::Mat small_img;
  cv::resize(gray, small_img, cv::Size(cvRound(frame.cols / m_dScale), cvRound(frame.rows / m_dScale)));
  ...
  cv::Mat recorte(frame, cv::Rect(cv::Point(cvRound(r.x * m_dScale), cvRound(r.y * m_dScale)),
  ...
  //DEBUG cv::imshow("detected"+(char)fx, recorte);
    ...
    if (1||DetectSubFace(detected)) {
      ...
      if (m_drawName) cv::putText(frame, "name", pt1e, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255, 0), 2);
      if (m_drawFrame) cv::rectangle(frame, pt1, pt2, CV_RGB(255, 0, 0), 1, 8, 0);
      ...
      if (areaDiff < 297000) {
        ...
        std::cout << "[" << m_camId << "]["<< fx<<"]: " << ((double)areaFace)/areaImage << " (" << t/((double)cvGetTickFrequency()*1000) << "ms)" << std::endl;
